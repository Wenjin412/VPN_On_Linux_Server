...
minimal Linux servers is predictable. Mihomo does the proxy work; vpnctl handles
...
SERVICE_NAME = os.environ.get("VPNCTL_SERVICE_NAME", "vpn-on-linux.service")
ETC_DIR = Path(os.environ.get("VPNCTL_ETC_DIR", "/etc/vpn-on-linux"))
VAR_DIR = Path(os.environ.get("VPNCTL_VAR_DIR", "/var/lib/vpn-on-linux"))
STATE_PATH = Path(os.environ.get("VPNCTL_STATE_PATH", str(ETC_DIR / "state.json")))
CONFIG_PATH = Path(os.environ.get("VPNCTL_CONFIG_PATH", str(ETC_DIR / "config.yaml")))
PROVIDER_PATH = Path(
    os.environ.get("VPNCTL_PROVIDER_PATH", str(ETC_DIR / "providers" / "subscription.yaml"))
)
...
def read_state(required: bool = False) -> dict[str, Any]:
    if not STATE_PATH.exists():
        if required:
            raise CliError(
                f"{STATE_PATH} does not exist. Run 'sudo vpnctl setup <subscription-url>' first."
            )
...
def need_subscription(state: dict[str, Any]) -> str:
    ...
    if not url:
        raise CliError("No subscription URL configured. Run 'sudo vpnctl setup <subscription-url>'.")
    return str(url)
...
def render_config_text(state: dict[str, Any]) -> str:
    ...
    lines = [
        "# Generated by vpnctl. Edit with vpnctl commands; manual edits may be overwritten.",
...
def cmd_setup(args: argparse.Namespace) -> None:
    ...
    if not args.quiet:
        print("VPN service is configured. Use 'vpnctl status' and 'vpnctl test'.")
...
def cmd_service(args: argparse.Namespace) -> None:
    action = args.command
    if action == "start":
        refresh_subscription(argparse.Namespace(quiet=True))
        render_config(argparse.Namespace(quiet=True))
...
def controller_request(
    ...
    try:
        ...
        raise CliError(
            "Cannot reach Mihomo controller. Is the service running? Try 'sudo vpnctl start'."
        ) from exc
    if not payload:
        return None
...
def cmd_nodes(args: argparse.Namespace) -> None:
    ...
    if nodes_command == "list":
        ...
        if not nodes:
            raise CliError("No nodes found. Try 'sudo vpnctl subscription refresh' or check service logs.")
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if not args.argv:
        raise CliError("Usage: vpnctl run -- <command> [args...]")
    state = read_state(required=True)
    env = os.environ.copy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"vpnctl: command failed ({exc.returncode}): {cmd}", file=sys.stderr)
